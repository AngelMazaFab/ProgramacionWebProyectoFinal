document.addEventListener('DOMContentLoaded', async () => {
    ...
    try {
        ...
        const emptyMsg = '<p style="text-align:center; color:var(--text-muted); margin-top: 4rem;">Sin datos registrados</p>';

        // ===== CHART: PIE (Citas por Estado) =====
        const ctxCitas = document.getElementById('chartCitas');
        if (ctxCitas) {
            if (!data.citas || data.citas.length === 0) {
                ctxCitas.parentElement.innerHTML = emptyMsg;
            } else {
                const estadoColors = {
        ...
        if (ctxConsultas) {
            if (!data.consultas || data.consultas.length === 0) {
                ctxConsultas.parentElement.innerHTML = emptyMsg;
            } else {
        ...
        if (ctxIngresos) {
            ...
            if (ingresosData.length === 0) {
                ctxIngresos.parentElement.innerHTML = emptyMsg;
            } else {
        ...
        if (ctxTendencia) {
            if (!data.tendencia_citas || data.tendencia_citas.length === 0) {
                ctxTendencia.parentElement.innerHTML = emptyMsg;
            } else {
